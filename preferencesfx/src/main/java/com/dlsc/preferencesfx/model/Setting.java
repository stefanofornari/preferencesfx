...
import com.dlsc.formsfx.view.controls.DoubleSliderControl;
import com.dlsc.formsfx.view.controls.IntegerSliderControl;
import com.dlsc.formsfx.view.controls.SimpleChooserControl;
import com.dlsc.formsfx.view.controls.SimpleColorPickerControl;
...
import com.dlsc.formsfx.view.controls.ToggleControl;
...
/**
 * Represents a setting, which holds the field to be displayed and the property
 * which is bound.
 *
...
public class Setting<E extends Element, P extends Property> {
    ...
    /**
     * Constructs a setting of {@link Boolean} type, which is represented by a
     * {@link ToggleControl}.
     *
    ...
    /**
     * Constructs a setting of {@link Integer} type, which is represented by a
     * {@link TextField}.
     *
     * @param description the title of this setting
     * @param property to be bound, saved / loaded and used for undo / redo
    ...
    /**
     * Constructs a setting of {@link Double} type, which is represented by a
     * {@link TextField}.
     *
     * @param description the title of this setting
     * @param property to be bound, saved / loaded and used for undo / redo
    ...
    /**
     * Constructs a setting of {@link String} type, which is represented by a
     * {@link TextField}.
     *
     * @param description the title of this setting
     * @param property to be bound, saved / loaded and used for undo / redo
     ...
     *
     * @param description the title of this setting
     * @param items the items which are possible to choose in the combobox,
     * which are shown in their {@link #toString()} representation
     * @param selection the currently selected item of the combobox to be bound,
     * saved / loaded and used for undo / redo
    ...
    /**
     * Creates a combobox with multiselection. At least one element has to be
     * selected at all times.
     *
     * @param description the title of this setting
     * @param items the items which are possible to choose in the combobox,
     * which are shown in their {@link #toString()} representation
     * @param selections the currently selected item(s) of the combobox to be
     * bound, saved / loaded and used for undo / redo
     * @param <P> the type of objects which should be displayed in the combobox
     * @return the constructed setting
     */
    public static <P> Setting of(
     ...
     * <br>
     * This allows for custom elements which just consist of a Node, without
     * showing a description.
     *
     * @apiNote Changed state of the {@link Node} will NOT be saved! Only use
     * this for {@link Node}s with static content!
     *
     ...
     *
     * @param description the title of this setting
     * @param fileProperty the property to which the chosen file / directory
     * should be set to
     ...
     * @param fileProperty the property to which the chosen file / directory
     * should be set to
     * @param initialDirectory An optional initial path, can be null. If null,
     * will use the path from the previously chosen file if present.
     * @param directory true, if only directories are allowed
     * @return the constructed setting
     */
    public static Setting of(String description,
     ...
     * field. Limited to validators that are able to handle the field's
     * underlying type.
     * @return the current setting to allow for chaining
     * @throws UnsupportedOperationException if this {@link Field} is not
     * instanceof {@link DataField}
     */
    @SafeVarargs
    ...
    /**
     * Marks a setting. Is used for the search, which marks and unmarks items
     * depending on the match as a form of visual feedback.
     */
    public void mark() {
        if (!hasDescription()) {
    ...
    /**
     * Unmarks a setting. Is used for the search, which marks and unmarks items
     * depending on the match as a form of visual feedback.
     */
    public void unmark() {
        if (!hasDescription()) {
    ...
    /**
     * Returns the description of this setting or if i18n is used, it will
     * return the translated description in the current locale.
     *
     ...
     *
     * @param storageHandler the {@link StorageHandler} to use
     * @implNote differentiates between a {@link ListProperty}, as found in
     * multiselection settings, and all the other property types, since those
     * need to be handled differently by the {@link StorageHandler}.
     */
    ...
    /**
     * Adds the {@code breadCrumb} to this breadcrumb and updates all of its
     ...
     * <br>
     * For example, if {@code element} is a {@link Field}, {@code value} is
     * defined, however if {@code element} is a
     * {@link NodeElement}, {@code value} will be null, since there is nothing
     * to persist.
     *
     ...
     * <br>
     * For example, if {@code element} is a {@link Field}, {@code description}
     * is defined, however if {@code element} is a
     * {@link NodeElement}, {@code description} will be null, since
    ...
    /**
     * Sets the Preference key to be used instead of the breadcrumb. Can be used
     * without hash in a custom {@link StorageHandler}.
     *
     ...
     * @param additionalVisibilityCondition define the visibility condition to
     * be added (ANDed) to the current (if any)
     */
